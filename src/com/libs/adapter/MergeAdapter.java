...
public class MergeAdapter extends BaseAdapter implements SectionIndexer {
    ...
    protected CascadeDataSetObserver dataSetObserver = new CascadeDataSetObserver();
    protected HashSet<ListAdapter> hidden = new HashSet<ListAdapter>();
    ...
    public void hideAdapter(int index) {
        if (index >= 0 && index < pieces.size()) {
            ListAdapter adapter = pieces.get(index);
            if (hidden.add(adapter)) {
    ...
    public void showAdapter(int index) {
        if (index >= 0 && index < pieces.size()) {
            ListAdapter adapter = pieces.get(index);
            if (hidden.remove(adapter)) {
    ...
    public void hideAdapter(ListAdapter adapter) {
        int index = pieces.indexOf(adapter);
        if (index != -1)
            hideAdapter(index);
    }
    
    public void showAdapter(ListAdapter adapter) {
        int index = pieces.indexOf(adapter);
        if (index != -1)
            showAdapter(index);
    }
...
//    public void addAdapter(int index, ListAdapter adapter) {
//        if (pieces.contains(adapter))
//            return;
...
//        if (index >= 0 && index < pieces.size()) {
//            ListAdapter adapter = pieces.remove(index);
//            adapter.unregisterDataSetObserver(dataSetObserver);
//            notifyDataSetChanged();
//        }
//    }
...
//    }
    ...
    public ArrayList<ListAdapter> getAllAdapters() {
        return pieces;
    }
